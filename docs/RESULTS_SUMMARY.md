...
- **Balanced Accuracy**: media del recall por clase; resulta útil cuando las clases están desbalanceadas.
- **Macro F1**: media no ponderada del F1 por clase; penaliza mejor los fallos en clases minoritarias.
- **Cohen's Kappa**: acuerdo corregido por azar.
- **Macro Recall**: media no ponderada del recall por clase.

## Conclusión técnica

Hemos observado que M3 ofrece el comportamiento más equilibrado si priorizamos Macro F1 y Balanced Accuracy. M6 alcanza la mejor Accuracy y el mejor Kappa, pero su rendimiento en métricas macro no supera al de M3.
...
- Grad-CAM del modelo seleccionado.
